...
${formatErrorKey(key)}: ${errorContext[key]}`;
  ...
  // src/layers/carto-tile-layer.ts
  (0, import_core.registerLoaders)([carto_vector_tile_loader_default]);
  ...
  var H3TileLayer = class extends import_core3.CompositeLayer {
    initializeState() {
  ...
  var QuadbinTileLayer = class extends import_core4.CompositeLayer {
    initializeState() {
      this.setState({
        data: null,
        tileJSON: null
...
`;
  ...
  function assert2(condition, message) {
    import_core5.log.assert(condition, message);
  }
  ...
  var RasterLayer = class extends import_core6.CompositeLayer {
    renderLayers() {
      ...
      assert2(blockWidth === blockHeight, `blockWidth (${blockWidth}) must equal blockHeight (${blockHeight})`);
  ...
  function getLayer(type, config, dataset) {
    ...
    assert2(layer, `Unsupported layer type: ${type}`);
    return {
      ...layer,
  ...
  function checkFetchLayerDataParameters({
    ...
    assert2(connection, "Must define connection");
    assert2(type, "Must define a type");
    assert2(source, "Must define a source");
    assert2(credentials2.apiVersion === API_VERSIONS.V3, "Method only available for v3");
    assert2(credentials2.apiBaseUrl, "Must define apiBaseUrl");
    assert2(credentials2.accessToken, "Must define an accessToken");
    if (columns) {
      assert2(COLUMNS_SUPPORT.includes(type), `The columns parameter is not supported by type ${type}`);
    }
    if (geoColumn) {
      assert2(GEO_COLUMN_SUPPORT.includes(type), `The geoColumn parameter is not supported by type ${type}`);
    } else {
      assert2(!aggregationExp, "Have aggregationExp, but geoColumn parameter is missing");
      assert2(!aggregationResLevel, "Have aggregationResLevel, but geoColumn parameter is missing");
    }
  }
  ...
  async function _fetchDataUrl({
    ...
    if (format2) {
      ...
      assert2(url, `Format ${format2} not available`);
    } else {
      ...
      assert2(url && mapFormat, "Unsupported data formats received from backend.");
    }
  ...
  async function fetchMap({
    ...
    assert2(cartoMapId, 'Must define CARTO map id: fetchMap({cartoMapId: "XXXX-XXXX-XXXX"})');
    assert2(localCreds.apiVersion === API_VERSIONS.V3, "Method only available for v3");
    assert2(localCreds.apiBaseUrl, "Must define apiBaseUrl");
    if (!localCreds.mapsUrl) {
      localCreds.mapsUrl = buildMapsUrlFromBase(localCreds.apiBaseUrl);
    }
    if (autoRefresh || onNewData) {
      ...
      assert2(typeof autoRefresh === "number" && autoRefresh > 0, "`autoRefresh` must be a positive number");
    }
